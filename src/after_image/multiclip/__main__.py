...
from .player import MulticlipPlayer, build_shim_tracks
...
EFFECT = os.environ.get('EFFECT', 'multi')


def draw_debug(
    display, recorder: SceneRecorder, player: MulticlipPlayer, fps: float
) -> None:
    mode = 'OVERLAY' if player.is_ready() else 'LIVE'
    rec = 'REC' if recorder.is_recording else 'IDLE'
    rec_color = (0, 0, 255) if recorder.is_recording else (200, 200, 200)
    cv2.putText(
        display, f'{mode}  effect={EFFECT}', (10, 30),
...
def main():
    if EFFECT not in RENDERERS:
        raise SystemExit(
            f'unknown EFFECT={EFFECT!r}, must be one of {list(RENDERERS)}'
        )
    ...
    print(f'camera: {actual_w}x{actual_h} @ {fps:.2f}fps  effect={EFFECT}')
    ...
    player = MulticlipPlayer()
    render = RENDERERS[EFFECT]

    cv2.namedWindow('after-image-multiclip', cv2.WINDOW_NORMAL)
    if not DEBUG:
        cv2.setWindowProperty(
            'after-image-multiclip',
    ...
    try:
        while True:
            ...
            if finished is not None:
                ...
                player.set_clips(list(recorder.stock))

            clip_frames = player.read_frames()
            if clip_frames:
                tracks = build_shim_tracks(clip_frames)
                display = render(frame, tracks)
            else:
                display = frame
            ...
            cv2.imshow('after-image-multiclip', display)
